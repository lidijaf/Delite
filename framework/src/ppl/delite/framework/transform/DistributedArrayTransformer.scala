...
trait DistributedArrayTransformer extends ForwardPassTransformer { 
  ...
  val distributedMode = false //TODO: config flag
  ...
  def markPartitioned[T](sym: Sym[T], v: Sym[Long], d: Def[T]) {
  ...
  def checkIllegalUsage[T](sym: Sym[T], d: Def[T]) {
    if (distributedMode && deliteKernel) { //allow arbitrary consumers for numa or if already inside a parallel op
