...
trait DeliteArrayBufferOps extends Base {
  ...
  object DeliteArrayBuffer {
    ...
    def apply[A:Manifest](initSize: Rep[Long])(implicit ctx: SourceContext) = darray_buffer_new(initSize)
    def apply[A:Manifest](data: Rep[DeliteArray[A]], length: Rep[Long])(implicit ctx: SourceContext) = darray_buffer_new_imm(data, length)
    def fromFunction[A:Manifest](size: Rep[Long])(func: Rep[Long] => Rep[A])(implicit ctx: SourceContext) = darray_buffer_from_function(size, func)
  }
  ...
  class DeliteArrayBufferOpsCls[A:Manifest](d: Rep[DeliteArrayBuffer[A]]) {
    ...
    def insert(pos: Rep[Long], elem: Rep[A])(implicit ctx: SourceContext) = darray_buffer_insert(d,pos,elem)
    def result(implicit ctx: SourceContext) = darray_buffer_result(d)
    def apply(idx: Rep[Long])(implicit ctx: SourceContext) = darray_buffer_apply(d,idx)
    def update(idx: Rep[Long], x: Rep[A])(implicit ctx: SourceContext) = darray_buffer_update(d,idx,x)
    ...
    def forIndices(func: Rep[Long] => Rep[Unit])(implicit ctx: SourceContext) = darray_buffer_forIndices(d,func)
  ...
  def darray_buffer_new[A:Manifest](initSize: Rep[Long])(implicit ctx: SourceContext): Rep[DeliteArrayBuffer[A]]
  def darray_buffer_new_imm[A:Manifest](data: Rep[DeliteArray[A]], length: Rep[Long])(implicit ctx: SourceContext): Rep[DeliteArrayBuffer[A]]
  def darray_buffer_from_function[A:Manifest](size: Rep[Long], func: Rep[Long] => Rep[A])(implicit ctx: SourceContext): Rep[DeliteArrayBuffer[A]]
  def darray_buffer_apply[A:Manifest](d: Rep[DeliteArrayBuffer[A]], idx: Rep[Long])(implicit ctx: SourceContext): Rep[A]
  def darray_buffer_update[A:Manifest](d: Rep[DeliteArrayBuffer[A]], idx: Rep[Long], x: Rep[A])(implicit ctx: SourceContext): Rep[Unit]
  def darray_buffer_length[A:Manifest](d: Rep[DeliteArrayBuffer[A]])(implicit ctx: SourceContext): Rep[Long]
  ...
  def darray_buffer_insert[A:Manifest](d: Rep[DeliteArrayBuffer[A]], pos: Rep[Long], elem: Rep[A])(implicit ctx: SourceContext): Rep[Unit]
  ...
  def darray_buffer_forIndices[A:Manifest](d: Rep[DeliteArrayBuffer[A]], func: Rep[Long] => Rep[Unit])(implicit ctx: SourceContext): Rep[Unit]
...
trait DeliteArrayBufferOpsExp extends DeliteArrayBufferOps with DeliteCollectionOpsExp with StructExp with PrimitiveOpsExp with EqualExp with VariablesExp with DeliteStructsExp {
  ...
  case class DeliteArrayBufferNew[A:Manifest](initSize: Exp[Long], logicalSize: Exp[Long])(implicit ctx: SourceContext) extends DeliteStruct[DeliteArrayBuffer[A]] {
    val elems = copyTransformedElems(Seq("data" -> var_new(DeliteArray[A](initSize)).e, "length" -> var_new(logicalSize).e))
    val mA = manifest[A]
  }

  case class DeliteArrayBufferNewImm[A:Manifest](data: Exp[DeliteArray[A]], length: Exp[Long]) extends DeliteStruct[DeliteArrayBuffer[A]] {
    val elems = copyTransformedElems(Seq("data" -> data, "length" -> length))
    val mA = manifest[A]
  }

  def darray_buffer_new[A:Manifest](initSize: Exp[Long])(implicit ctx: SourceContext): Exp[DeliteArrayBuffer[A]] = darray_buffer_new(initSize, unit(0))

  def darray_buffer_new[A:Manifest](initSize: Exp[Long], logicalSize: Exp[Long])(implicit ctx: SourceContext): Exp[DeliteArrayBuffer[A]] = reflectMutable(DeliteArrayBufferNew(initSize, logicalSize))

  def darray_buffer_new_imm[A:Manifest](data: Exp[DeliteArray[A]], length: Exp[Long])(implicit ctx: SourceContext): Exp[DeliteArrayBuffer[A]] = reflectPure(DeliteArrayBufferNewImm(data, length))

  def darray_buffer_apply[A:Manifest](d: Exp[DeliteArrayBuffer[A]], idx: Exp[Long])(implicit ctx: SourceContext): Exp[A] = darray_buffer_raw_data(d).apply(idx)

  def darray_buffer_update[A:Manifest](d: Exp[DeliteArrayBuffer[A]], idx: Exp[Long], x: Exp[A])(implicit ctx: SourceContext) = darray_buffer_raw_data(d).update(idx,x)
  ...
  def darray_buffer_length[A:Manifest](d: Exp[DeliteArrayBuffer[A]])(implicit ctx: SourceContext) = field[Long](d, "length")
  protected def darray_buffer_set_length[A:Manifest](d: Exp[DeliteArrayBuffer[A]], len: Exp[Long]) = field_update[Long](d, "length", len)

  def darray_buffer_insert[A:Manifest](d: Exp[DeliteArrayBuffer[A]], pos: Exp[Long], x: Exp[A])(implicit ctx: SourceContext): Exp[Unit] = {
    darray_buffer_insertspace(d,pos,unit(1))
    darray_buffer_update(d,pos,x)
  }

  protected def darray_buffer_insertAll[A:Manifest](d: Exp[DeliteArrayBuffer[A]], pos: Exp[Long], xs: Exp[DeliteArray[A]])(implicit ctx: SourceContext): Exp[Unit] = {
    darray_buffer_insertspace(d,pos,xs.length)
    darray_buffer_copyfrom(d, pos, xs)
  }

  protected def darray_buffer_copyfrom[A:Manifest](d: Exp[DeliteArrayBuffer[A]], pos: Exp[Long], xs: Exp[DeliteArray[A]]): Exp[Unit] = {
    val data = darray_buffer_raw_data(d)
    darray_copy(xs, unit(0), data, pos, xs.length)
  }

  protected def darray_buffer_insertspace[A:Manifest](d: Exp[DeliteArrayBuffer[A]], pos: Exp[Long], len: Exp[Long]): Exp[Unit] = {
  ...
  protected def darray_buffer_ensureextra[A:Manifest](d: Exp[DeliteArrayBuffer[A]], extra: Exp[Long]): Exp[Unit] = {
    val data = darray_buffer_raw_data(d)
  ...
  protected def darray_buffer_realloc[A:Manifest](d: Exp[DeliteArrayBuffer[A]], minLen: Exp[Long]): Exp[Unit] = {  
    val oldData = darray_buffer_raw_data(d)
    val doubleLength = oldData.length * unit(2)
    val n = var_new(if (doubleLength < unit(16l)) unit(16l) else doubleLength)
  ...
  case class DeliteArrayBufferMap[A:Manifest,B:Manifest](in: Exp[DeliteArrayBuffer[A]], func: Exp[A] => Exp[B])(implicit ctx: SourceContext)
    ...
    override def alloc(len: Exp[Long]) = darray_buffer_new[B](len,len) //flat
  }

  case class DeliteArrayBufferMapIndices[A:Manifest](size: Exp[Long], func: Exp[Long] => Exp[A])
    extends DeliteOpMapIndices[A,DeliteArrayBuffer[A]] {

    override def alloc(len: Exp[Long]) = darray_buffer_new[A](len,len) //flat
  }
  ...
  case class DeliteArrayBufferZipWith[A:Manifest,B:Manifest,R:Manifest](inA: Exp[DeliteArrayBuffer[A]], inB: Exp[DeliteArrayBuffer[B]], func: (Exp[A], Exp[B]) => Exp[R])
    ...
    override def alloc(len: Exp[Long]) = darray_buffer_new[R](len,len) //flat
  ...
  case class DeliteArrayBufferFilter[A:Manifest](in: Exp[DeliteArrayBuffer[A]], cond: Exp[A] => Exp[Boolean])
    ...
    override def alloc(len: Exp[Long]) = darray_buffer_new[A](len) //buffer
    val size = copyTransformedOrElse(_.size)(in.length)
    def func = e => e
  }
  ...
  case class DeliteArrayBufferFlatMap[A:Manifest,B:Manifest](in: Exp[DeliteArrayBuffer[A]], func: Exp[A] => Exp[DeliteArrayBuffer[B]])
    ...
    override def alloc(len: Exp[Long]) = darray_buffer_new[B](len) //buffer
    val size = copyTransformedOrElse(_.size)(in.length)
  }
  ...
  case class DeliteArrayBufferGroupBy[A:Manifest,K:Manifest](in: Exp[DeliteArrayBuffer[A]], keyFunc: Exp[A] => Exp[K]) 
    ...
    def alloc(len: Exp[Long]) = DeliteArray[DeliteArrayBuffer[A]](len)
    def allocI(len: Exp[Long]) = darray_buffer_new[A](len) //buffer
    val size = copyTransformedOrElse(_.size)(in.length)
  }
  ...
  case class DeliteArrayBufferForIndices[A:Manifest](in: Exp[DeliteArrayBuffer[A]], func: Rep[Long] => Rep[Unit])(implicit ctx: SourceContext) extends DeliteOpIndexedLoop {
    val size = copyTransformedOrElse(_.size)(in.length)
    val mA = manifest[A]
  }
  ...
  def darray_buffer_from_function[A:Manifest](size: Exp[Long], func: Exp[Long] => Exp[A])(implicit ctx: SourceContext) = reflectPure(DeliteArrayBufferMapIndices(size,func))
  ...
  def darray_buffer_forIndices[A:Manifest](d: Rep[DeliteArrayBuffer[A]], func: Rep[Long] => Rep[Unit])(implicit ctx: SourceContext) = {
  ...
  override def unapplyStructType[T:Manifest]: Option[(StructTag[T], List[(String,Manifest[_])])] = manifest[T] match {
    case t if t.erasure == classOf[DeliteArrayBuffer[_]] => Some((classTag(t), List("data","length") zip List(darrayManifest(t.typeArguments(0)), manifest[Long]))) 
