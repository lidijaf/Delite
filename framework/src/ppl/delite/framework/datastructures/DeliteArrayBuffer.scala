...
trait DeliteArrayBufferOpsExp extends DeliteArrayBufferOps with DeliteCollectionOpsExp with StructExp with PrimitiveOpsExp with EqualExp with VariablesExp with DeliteStructsExp {
  ...
  case class DeliteArrayBufferNew[A:Manifest](initSize: Exp[Long], logicalSize: Exp[Long]) extends DeliteStruct[DeliteArrayBuffer[A]] {
  ...
  case class DeliteArrayBufferForIndices[A:Manifest](in: Exp[DeliteArrayBuffer[A]], func: Rep[Long] => Rep[Unit]) extends DeliteOpIndexedLoop {
    val size = copyTransformedOrElse(_.size)(in.length)
    val mA = manifest[A]
  }
