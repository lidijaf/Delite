...
// trait DeliteArrayNuma[T] extends DeliteCollection[T] 

// trait DeliteArrayNumaOps extends Base {

//   var partitionArray: Boolean = false
  
//   object DeliteArrayNuma {
//     def apply[T:Manifest](length: Rep[Int])(implicit ctx: SourceContext) = darray_numa_new(length)
//   }

//   implicit def repDArrayNumaToDArrayNumaOps[T:Manifest](da: Rep[DeliteArrayNuma[T]])(implicit ctx: SourceContext) = new DeliteArrayNumaOpsCls(da)

//   class DeliteArrayNumaOpsCls[T:Manifest](da: Rep[DeliteArrayNuma[T]])(implicit ctx: SourceContext) {
//     def length: Rep[Int] = darray_numa_length(da)
//     def apply(i: Rep[Int]): Rep[T] = darray_numa_apply(da,i)
//     def update(i: Rep[Int], x: Rep[T]): Rep[Unit] = darray_numa_update(da,i,x)
//     // def mutable = darray_numa_mutable(da)
...
//     def foreach(f: Rep[T] => Rep[Unit]) = darray_numa_foreach(da,f)
...
//   def darray_numa_new[T:Manifest](length: Rep[Int])(implicit ctx: SourceContext): Rep[DeliteArrayNuma[T]]
//   // def darray_new_immutable[T:Manifest](length: Rep[Int])(implicit ctx: SourceContext): Rep[DeliteArray[T]]
//   def darray_numa_length[T:Manifest](da: Rep[DeliteArrayNuma[T]])(implicit ctx: SourceContext): Rep[Int]
//   def darray_numa_apply[T:Manifest](da: Rep[DeliteArrayNuma[T]], i: Rep[Int])(implicit ctx: SourceContext): Rep[T]
//   def darray_numa_update[T:Manifest](da: Rep[DeliteArrayNuma[T]], i: Rep[Int], x: Rep[T])(implicit ctx: SourceContext): Rep[Unit]
//   // def darray_numa_mutable[T:Manifest](d: Rep[DeliteArrayNuma[T]])(implicit ctx: SourceContext): Rep[DeliteArrayNuma[T]]
...
//   def darray_numa_foreach[A:Manifest](d: Rep[DeliteArrayNuma[A]], f: Rep[A] => Rep[Unit])(implicit ctx: SourceContext): Rep[Unit]
...
// trait DeliteArrayNumaOpsExp extends DeliteArrayNumaStructTags with DeliteCollectionOpsExp with DeliteStructsExp with RuntimeServiceOpsExp {
//   this: DeliteOpsExp with DeliteMapOpsExp =>
  
//   //////////////////
//   // codegen ops

//   case class DeliteArrayNumaNew[T](length: Exp[Int], m:Manifest[T], tag: PartitionTag[T]) extends Def[DeliteArrayNuma[T]] //pass in manifest explicitly so it becomes part of equality (cse) check
//   case class DeliteArrayNumaLength[T:Manifest](da: Exp[DeliteArrayNuma[T]]) extends DefWithManifest[T,Int]
...
//   case class DeliteArrayNumaForeach[A:Manifest](in: Exp[DeliteArrayNuma[A]], func: Rep[A] => Rep[Unit]) extends DeliteOpForeach[A] {
...
//   def isDeliteArrayNuma[A](x: Exp[DeliteCollection[A]])(implicit ctx: SourceContext) = isSubtype(x.tp.erasure,classOf[DeliteArrayNuma[A]])  
//   def asDeliteArrayNuma[A](x: Exp[DeliteCollection[A]])(implicit ctx: SourceContext) = x.asInstanceOf[Exp[DeliteArrayNuma[A]]]
    
//   override def dc_size[A:Manifest](x: Exp[DeliteCollection[A]])(implicit ctx: SourceContext) = { 
//     if (isDeliteArrayNuma(x)) asDeliteArrayNuma(x).length
//     else super.dc_size(x)
//   }
  
//   override def dc_apply[A:Manifest](x: Exp[DeliteCollection[A]], n: Exp[Int])(implicit ctx: SourceContext) = {
//     if (isDeliteArrayNuma(x)) asDeliteArrayNuma(x).apply(n)
//     else super.dc_apply(x,n)
//   }
  
//   override def dc_update[A:Manifest](x: Exp[DeliteCollection[A]], n: Exp[Int], y: Exp[A])(implicit ctx: SourceContext) = {
//     if (isDeliteArrayNuma(x)) asDeliteArrayNuma(x).update(n,y)
//     else super.dc_update(x,n,y)        
//   }

//   // override def dc_set_logical_size[A:Manifest](x: Exp[DeliteCollection[A]], y: Exp[Int])(implicit ctx: SourceContext) = {
//   //   if (isDeliteArrayNuma(x)) {
//   //     val arr = asDeliteArrayNuma(x)
//   //     if (delite_greater_than(arr.length, y)) { //trim
//   //       val newArr = DeliteArrayNuma[A](y)
...
//     if (isDeliteArrayNuma(x)) { unit(true) }
...
//     if (isDeliteArrayNuma(x)) DeliteArrayNuma[A](size).asInstanceOf[Exp[CA]]
...
//   //   if (isDeliteArrayNuma(src) && isDeliteArrayNuma(dst)) {
...
//   def darray_numa_new[T:Manifest](length: Exp[Int])(implicit ctx: SourceContext) = reflectMutable(DeliteArrayNumaNew(length,manifest[T],PartitionTag("DeliteArrayNuma",partitionArray)))
