...
trait DeliteFileReaderOpsExp extends DeliteFileReaderOps with DeliteArrayOpsExpOpt with DeliteArrayBufferOpsExp with DeliteOpsExp with DeliteMapOpsExp {
  ...
  case class DeliteFileStreamReadBytes(stream: Exp[DeliteFileStream], idx: Exp[Long]) extends Def[DeliteArray[Byte]]
  def dfs_readBytes(stream: Exp[DeliteFileStream], idx: Exp[Long])(implicit pos: SourceContext): Exp[DeliteArray[Byte]] = reflectPure(DeliteFileStreamReadBytes(stream, idx))
  ...
  abstract class DeliteOpFileReaderFlatI[A:Manifest, I<:DeliteCollection[A]:Manifest, CA<:DeliteCollection[A]:Manifest]
    ...
    def func: Exp[Long] => Exp[DeliteCollection[A]]
