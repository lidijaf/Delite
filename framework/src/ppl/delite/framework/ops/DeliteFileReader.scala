...
trait DeliteFileReaderOps extends Base with DeliteArrayBufferOps {
  ...
  object DeliteFileReader {
    ...
    //def readBytesFlattened[A:Manifest](paths: Rep[String]*)(delimiter: Rep[DeliteArray[Byte]])(f: Rep[DeliteArray[Byte]] => Rep[DeliteCollection[A]])(implicit pos: SourceContext) = dfr_readBytesFlattened(paths, delimiter, f)
  }
  ...
  def dfs_new(paths: Seq[Rep[String]], delimiter: Rep[DeliteArray[Byte]] = unit(null))(implicit pos: SourceContext): Rep[DeliteFileStream]
...
trait DeliteFileReaderOpsExp extends DeliteFileReaderOps with DeliteArrayOpsExpOpt with DeliteArrayBufferOpsExp with DeliteOpsExp with DeliteMapOpsExp {
  ...
  case class DeliteFileStreamNew(paths: Seq[Exp[String]], delimiter: Rep[DeliteArray[Byte]]) extends Def[DeliteFileStream]
  def dfs_new(paths: Seq[Exp[String]], delimiter: Rep[DeliteArray[Byte]])(implicit pos: SourceContext) = reflectPure(DeliteFileStreamNew(paths, delimiter))

  case class DeliteFileStreamReadLine(stream: Exp[DeliteFileStream], idx: Exp[Long]) extends Def[String]
  def dfs_readLine(stream: Exp[DeliteFileStream], idx: Exp[Long])(implicit pos: SourceContext): Exp[String] = reflectPure(DeliteFileStreamReadLine(stream, idx))
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit ctx: SourceContext): Exp[A] = (e match {
    ...
    case DeliteFileStreamNew(paths,delim) => dfs_new(f(paths),f(delim))
    case Reflect(DeliteFileStreamNew(paths,delim), u, es) => reflectMirrored(Reflect(DeliteFileStreamNew(f(paths),f(delim)), mapOver(f,u), f(es)))(mtype(manifest[A]), ctx)
...
trait ScalaGenDeliteFileReaderOps extends ScalaGenFat {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case DeliteFileStreamNew(paths, delimiter) =>
      emitValDef(sym, "generated.scala.io.FileStreamImpl(" + quote(delimiter) + "," + paths.map(quote).mkString(",") + ")")
...
trait CGenDeliteFileReaderOps extends CGenFat {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case DeliteFileStreamNew(paths, Const(null)) => 
    ...
    case DeliteFileStreamNew(paths, delimiter) => 
      throw new GenerationFailedException("FileReader: custom delimiter is not suppported by C codegen")
    case DeliteFileStreamReadLine(stream,idx) =>
