...
trait NestedLoopMappingExp extends Expressions {
  ...
  case class LoopConstraint(d: Dimension => Boolean, b: Long => Boolean, s: Span => Boolean, w: Double = 1.0)
  ...
  case class SpanOne(s: Exp[Long]) extends Span { override def toString(quote: Exp[Any] => String) = "\"span\":{\"tpe\":\"one\",\"size\":\""+quote(s)+"\"}" }
  case class SpanN(s: Exp[Long], n: Long) extends Span { override def toString(quote: Exp[Any] => String) = "\"span\":{\"tpe\":\"n\",\"size\":\""+quote(s)+"\",\"factor\":\""+n+"\"}" }
  case class Split(s: Exp[Long], k: Long) extends Span { override def toString(quote: Exp[Any] => String) = "\"span\":{\"tpe\":\"split\",\"size\":\""+quote(s)+"\",\"factor\":\""+k+"\"}" }
  ...
  def blockSizeConstraint(c: Long => Boolean) = LoopConstraint(_ => true, c, _ => true)

  // result of the analysis
  val loopAnalysisResult = new HashMap[Int, (Dimension,Long,Span)]
}
...
trait NestedLoopMappingAnalysis extends FatBlockTraversal with LoopFusionOpt with LoopSoAOpt with BaseGenDeliteArrayOps with BaseDeliteOpsTraversalFat {
  ...
  val hardConstraints = new HashMap[Long, HashSet[LoopConstraint]]()

  private def addHardC(hcIdx: Long, to: LoopInfo): Unit = {
    val c = hcIdx match {
  ...
  private val iWeights = Map[Long,Double](1l->10.0, 2l->5.0, 3l->2.0)

  private def addSoftC(scIdx: Long, to: LoopInfo, at: LoopInfo): Unit = {
    val c = scIdx match {
      case 1 =>
  ...
  private def infix_size(m: LoopInfo): Exp[Long] = {
    m.loop match {
  ...
  private def infix_index(m: LoopInfo): Exp[Long] = {
    m.loop match {
  ...
  private def nestedSize(m: LoopInfo, head: LoopInfo = loopInfoRoot): Option[Long] = {
    def loopSize(l: LoopInfo): Long = {
      l.size match {
        case Const(c) => c
        case _ => defaultLoopSize
      }
  ...
  object LoopIndex {
    def unapply(i: Exp[Long]): Option[LoopInfo] = {
      allLoops(loopInfoRoot).find(_.index == i)
    }
  }

  private def processArrayAccess(i: Exp[Long]) {
    printInfo("processing array access " + quote(i))
    i match {
      case LoopIndex(l) =>
        addSoftC(1, l, currentLoop)
      case Def(LongPlus(_,LoopIndex(l))) =>
        addSoftC(1, l, currentLoop)
      case Def(LongPlus(LoopIndex(l),_)) =>
        addSoftC(1, l, currentLoop)
      /*
  ...
  def calculateDOP(mapping: HashMap[Int,(Dimension,Long,Span)]): Double = {
    var dop: Double = 1.0
    for((k,v) <- mapping) {
      val s = v._3 match {
  ...
  def generateOptMapping: Unit = {
    ...
    val permSet = HashSet[(Dimension,Long,Span)]()
      ...
      for(l <- 0 until maxLevel) {
        ...
        span match {
          ...
          case SpanOne =>
            ...
            if(!kernelInputs.contains(sizes.head) && !sizes.head.isInstanceOf[Const[Long]]) pass = false
          case _ => //
        }
