...
trait DeliteOpsExpIR extends BaseFatExp with EffectExp with VariablesExp with LoopsFatExp with FunctionBlocksExp with IfThenElseFatExp
    with PrimitiveOpsExp with DeliteReductionOpsExp with StencilExp 
   ...
   *
   * Where should these live? TODO:vsalvis: required by stuff in this file :(
   */
...
*/
  ...
  abstract class DeliteOpAbstractLoop[A:Manifest](implicit ctx: SourceContext) extends AbstractLoop[A] with DeliteOp[A] {
    type OpType <: DeliteOpAbstractLoop[A]
  }
  ...
  // TR: shouldn't this be part of the codegen hierarchy?
  // used in DeliteCodegen and DeliteScalaGenIfThenElse, where it's imported
  // directly from DeliteOpsExp... Otherwise could put in BaseGenDeliteOps.
  ...
  override def reflectMirrored[A:Manifest](zd: Reflect[A])(implicit pos: SourceContext): Exp[A] = zd match {
    ...
    case Reflect(x: DeliteOpAbstractLoop[_], u, es) =>
...
//    case op: DeliteForeachElem[_] => freqNormal(op.sync) ::: freqHot(op.cond) ::: freqHot(op.func)
  ...
  override def copySyms(e: Any): List[Sym[Any]] = e match {
    case s: DeliteOpAbstractSingleTask[_] => Nil
    case e: DeliteOpAbstractExternal[_] => syms(e.allocVal)
