...
trait TableOps extends Base { this: OptiQL =>
  ...
  object Table {
	  def apply[T:Manifest](initSize: Rep[Long]): Rep[Table[T]] = tableObjectApply(initSize)
	  def apply[T:Manifest](data: Rep[DeliteArray[T]], size: Rep[Long]): Rep[Table[T]] = tableObjectApply(data, size)
    ...
    def range(start: Rep[Long], end: Rep[Long]): Rep[Table[Long]] = tableObjectRange(start, end)
  }

  class TableRepOps[T:Manifest](t:Rep[Table[T]]) {
    def apply(i: Rep[Long]): Rep[T] = tableApply(t, i)
  ...
  def tableApply[T:Manifest](t: Rep[Table[T]], i: Rep[Long]): Rep[T]
  def tableObjectApply[T:Manifest](): Rep[Table[T]]
  def tableObjectApply[T:Manifest](initSize: Rep[Long]): Rep[Table[T]]
  def tableObjectApply[T:Manifest](data: Rep[DeliteArray[T]], size: Rep[Long]): Rep[Table[T]]
  def tableSize[T:Manifest](t: Rep[Table[T]]): Rep[Long]
  def tableObjectRange(start: Rep[Long], end: Rep[Long]): Rep[Table[Long]]
...
trait TableOpsExp extends TableOps with DeliteCollectionOpsExp with DeliteStructsExp { this: OptiQLExp =>
  ...
  def tableSize[T:Manifest](t: Exp[Table[T]]): Exp[Long] = field[Long](t, "size")
  def tableApply[T:Manifest](t: Exp[Table[T]], i: Exp[Long]): Exp[T] = tableRawData(t).apply(i)

  def tableObjectApply[T:Manifest](): Exp[Table[T]] = tableObjectApply(unit(16))
  def tableObjectApply[T:Manifest](initSize: Exp[Long]): Exp[Table[T]] = struct(classTag[Table[T]], "data" -> fatal(unit("Table allocation within Delite Op not rewritten")), "size" -> initSize)
  def tableObjectApply[T:Manifest](data: Exp[DeliteArray[T]], size: Exp[Long]): Exp[Table[T]] = struct(classTag[Table[T]], "data" -> data, "size" -> size)

  def tableObjectRange(start: Exp[Long], end: Exp[Long]) = Table(DeliteArray.fromFunction(end-start)(i => i + start))
  ...
  override def unapplyStructType[T:Manifest]: Option[(StructTag[T], List[(String,Manifest[_])])] = {
    val m = manifest[T]
    if (m.erasure == classOf[Table[_]]) Some((classTag(m), List("data" -> darrayManifest(m.typeArguments(0)), "size" -> manifest[Long])))
